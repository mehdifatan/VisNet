...
def apply_local_inhibition_and_sparseness(x, sparsity, layer_radius, layer_size, device):
    """
    Apply global sparsity inhibition per sample.
    
    Args:
        x: Activations [batch_size, num_neurons]
        sparsity: Fraction to suppress (e.g., 0.99 keeps top 1%)
        layer_radius: Unused (kept for compatibility)
        layer_size: Unused (kept for compatibility)
        device: Unused (kept for compatibility)
    
    Returns:
        Sparse activations with non-winners zeroed
    """
    if x.dim() == 1:
        x = x.unsqueeze(0)
    if x.dim() != 2:
    ...
    n_features = x.size(1)
    keep = int((1.0 - float(sparsity)) * n_features)
    keep = max(1, min(keep, n_features))

    vals, _ = torch.topk(x, keep, dim=1, largest=True, sorted=False)
...
class L1Layer(nn.Module):
    ...
    def hebbian_update(self, x, y):
        ...
        for i in range(self.weights.size(0)):
            ...
            yi = y[:, i:i+1]
            # Cylinder-style masked pre/post interaction: Δw ∝ <post * pre_local>.
