...
def apply_local_inhibition_and_sparsity(activations: torch.Tensor, keep_ratio: float = 0.02) -> torch.Tensor:
    """Keep only the strongest fraction of activations per sample."""
...
class HebbianLayer2d(nn.Module):
    ...
    def hebbian_update(self, patches: torch.Tensor, activations: torch.Tensor, lr: float) -> None:
        ...
        inputs_norm = l2_normalize(patches)
        with torch.no_grad():
            updates = flat_activations.T @ inputs_norm
            updates = updates / max(1, patches.size(0))
            self.weights += lr * updates
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--keep-ratio", type=float, default=0.02, help="Fraction of neurons kept per layer.")
