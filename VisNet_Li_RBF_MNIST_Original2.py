...
def update_weights_2d_circular2(SynMat, Rate_old, input_data, Rate, Learnrate, alpha):
    ...
    dw=Learnrate*(pa)/batch_size
    
    
    #pp = pp.view(-1)
    #aa = aa.view(-1)
    #pa = compute_covariance(pp, pp)    
    
    #print(aa.shape)
    #print(SynMat.shape)
    #print(SynMat.shape)
    #pa=torch.mul(pp-SynMat,aa).sum(dim=0)
    #print(pa.shape)
    #dw=Learnrate*(pa)/batch_size
    #dw=Learnrate*(dw)/batch_size
    #print(dw.shape)
...
def update_weights_2d_circular2_L1(SynMat, Rate_old, input_data, Rate, Learnrate, alpha):
    ...
    aa = aa .unsqueeze(2)
    aa = aa.repeat(1,1,32*input_size[0]*input_size[1])
    aa = aa*all_mask_L1
    #pp = pp.view(-1)
    #aa = aa.view(-1)
    #SynMat2=SynMat.view(-1).clone()
    #pa = compute_covariance(pp, pp)   
...
def competitive_layer_2d(input_data, Rate_old, SynMat, Learnrate, OutputSparseness, alpha, layer):
    ...
    if layer==1:
        ...
        Actvn = compute_activations_rbf_elementwise_L1(input_data, SynMat)
    else:
        #Actvn = compute_activations_2d_circular(input_data, SynMat)
        Actvn = compute_activations_rbf_elementwise(input_data, SynMat)


    if layer==1:
        Rate = apply_local_inhibition_and_sparseness(Actvn, OutputSparseness)
    else:
        Rate = apply_local_inhibition_and_sparseness(Actvn, OutputSparseness)
    ...
    if layer==1:
        ...
        SynMat_updated = update_weights_2d_circular2_L1(SynMat, Rate_old, input_data, Rate, Learnrate, alpha)
    else:
        SynMat_updated = update_weights_2d_circular2(SynMat, Rate_old, input_data, Rate, Learnrate, alpha)
        
        
    return torch.nn.functional.relu(Rate), SynMat_updated
...
def process_images_through_competitive_layers_2d_circular(images, gabor_filters, OutputSparseness, SynMat1, SynMat2, SynMat3):
    ...
    # Compute activations for the 2D layer
    Actvn = compute_activations_rbf_elementwise_L1(gabor_processed, SynMat1)
    ...
    Actvn = compute_activations_rbf_elementwise(Actvn, SynMat2)
    ...
    Actvn = compute_activations_rbf_elementwise(Actvn, SynMat3)
    ...
    Actvn = compute_activations_rbf_elementwise(Actvn, SynMat4)
...
for epoch in range(n_epochs):
    ...
    for iter, data in enumerate(train_loader, 0):
        ...
        gabor_processed = preprocess_images(images[:,0,:,:], gabor_filters)
...
with torch.no_grad():
    ...
    for iter, data in enumerate(test_loader, 0):
        ...
        if images.dim() == 4 and images.size(1) == 1:
            ...
            # Already 2D or 3D
            images_2d = images
        activations,l4,l3,l2,l1 = process_images_through_competitive_layers_2d_circular(images_2d, gabor_filters, OutputSparseness, SynMat1, SynMat2, SynMat3)
