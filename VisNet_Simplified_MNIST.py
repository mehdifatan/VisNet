...
# ============================================================================
# GLOBAL SPARSITY INHIBITION (Original-style)
# ============================================================================

def apply_global_inhibition_and_sparsity(
    x: torch.Tensor, keep_ratio: float = 0.05
) -> torch.Tensor:
    """
    Apply global inhibition by keeping only the top fraction of responses per sample.

    Args:
        x: Activations [batch_size, num_neurons]
        keep_ratio: Fraction of activations to keep (e.g., 0.01 keeps top 1% of neurons)

    Returns:
        Sparse activations after zeroing out the rest.
    """
    if keep_ratio <= 0 or keep_ratio >= 1:
        return x
    keep = max(1, int(x.size(1) * keep_ratio))
    kth = x.size(1) - keep + 1
    threshold = torch.kthvalue(x, kth, dim=1, keepdim=True)[0]
...
class L1Layer(nn.Module):
    ...
    def forward(self, x):
        ...
        x = apply_global_inhibition_and_sparsity(x, keep_ratio=(1 - self.sparsity))
    ...
    def hebbian_update(self, x, y):
        """Masked pre/post Hebbian update (Original-style cylinder interaction)."""
        for i in range(self.weights.size(0)):
            # Get local receptive field for this neuron
            local_input = self.extract_local_receptive_field(x, i)
            
            yi = y[:, i:i+1]
            hebb = torch.mean(yi * local_input, dim=0, keepdim=True)
            weight_update = self.eta * hebb - self.decay * self.weights[i:i+1]
            
            # Update weights
            with torch.no_grad():
                self.weights[i:i+1] += weight_update
                self.weights[i:i+1] = F.normalize(self.weights[i:i+1], p=2, dim=1)
...
class VisNetWithGabor(nn.Module):
    ...
    def forward(self, x):
        ...
        # Apply Gabor filters to every color channel (DoG removed in this simplified variant)
        filtered_outputs = []
